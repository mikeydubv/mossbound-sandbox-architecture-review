...
function footprintCells(point, kind, rotation=0) {
  ...
  for (let x=0; x<w; x++) for (let z=0; z<d; z++) cells.push(`${Math.round((ox+x*GRID_SIZE)/GRID_SIZE)},${Math.round((oz+z*GRID_SIZE)/GRID_SIZE)}`);
  return cells;
}
function placementValid(point, kind, rotation, occupied) { return footprintCells(point, kind, rotation).every((key) => !occupied.has(key)); }
...
export default {
  ...
  async setup({ renderer, scene, camera, controls }) {
    ...
    const editor = { selected: null, pendingKind: null, terrainMode: null, helper: null, stream, inputs: null, apply() { if (!this.selected) return; const a = this.selected; a.scale.setScalar(Number(this.inputs.scale.value)); a.position.y = Number(this.inputs.y.value); a.rotation.y = Number(this.inputs.rot.value); if (a.userData.assetType === "lamp" && a.userData.pointLight) { a.userData.pointLight.intensity = Number(this.inputs.lightIntensity.value); a.userData.pointLight.distance = Number(this.inputs.lightRange.value); } a.traverse((o) => { if (o.material?.roughness !== undefined) o.material.roughness = Number(this.inputs.rough.value); if (o.material?.color && this.inputs.color.value) o.material.color.set(this.inputs.color.value); }); if (a.userData.assetType === "rock" && a.isMesh && Number(this.inputs.detail.value) !== a.userData.detail) { const next = createChippedRockGeometry(Number(this.inputs.detail.value)); a.geometry.dispose(); a.geometry = next; a.userData.detail = Number(this.inputs.detail.value); } this.helper?.update(); }, select(object) { if (this.helper) scene.remove(this.helper); this.selected = object; this.helper = new THREE.BoxHelper(object, 0x7ffff0); scene.add(this.helper); const box = new THREE.Box3().setFromObject(object); const center = box.getCenter(new THREE.Vector3()); this.inputs.name.textContent = `SELECTED: ${object.userData.assetName || `asset @ ${center.x.toFixed(1)}, ${center.z.toFixed(1)}`} · ${object.userData.footprint ? object.userData.footprint.join("×") : "1×1"} tiles`; this.inputs.name.style.color = "#7ffff0"; this.inputs.controls.style.display = "block"; this.inputs.lightIntensity.parentElement.style.display = object.userData.pointLight ? "grid" : "none"; this.inputs.lightRange.parentElement.style.display = object.userData.pointLight ? "grid" : "none"; this.inputs.streamY.parentElement.style.display = object === stream ? "grid" : "none"; this.inputs.speed.parentElement.style.display = object === stream ? "grid" : "none"; this.inputs.scale.value = object.scale.x; this.inputs.y.value = object.position.y; this.inputs.rot.value = object.rotation.y; const first = object.getObjectByProperty("isMesh", true); if (first?.material?.color) this.inputs.color.value = `#${first.material.color.getHexString()}`; }, deleteSelected() { if (!this.selected || this.selected === stream) return; this.selected.userData.gridCells?.forEach((key)=>occupied.delete(key)); this.selected.parent?.remove(this.selected); this.selected.visible=false; this.selected.traverse((o) => { o.geometry?.dispose?.(); if (Array.isArray(o.material)) o.material.forEach((m) => m.dispose?.()); else o.material?.dispose?.(); }); if (this.helper) scene.remove(this.helper); this.helper = null; this.selected = null; this.inputs.name.textContent = "Asset deleted"; this.inputs.name.style.color = "#ffc875"; this.inputs.controls.style.display = "none"; }, spawn(kind, point = null) { if (kind === "stream" && !point) { this.pendingKind = kind; this.inputs.name.textContent = "STREAM: click grid cells to place"; updatePreview(null,null); return; } if (!point) { this.pendingKind = kind; this.inputs.name.textContent = `PLACEMENT: click a grid tile for ${kind} · ${getSpec(kind).footprint.join("×")}`; updatePreview(null,null); return; } if (!placementValid(point,kind,0,occupied)) { this.inputs.name.textContent = "PLACEMENT BLOCKED: occupied tile"; this.inputs.name.style.color="#ff8090"; return; } const mat = new THREE.MeshStandardMaterial({ color: kind === "rock" ? 0x59615a : kind === "crate" ? 0x8a5a34 : kind === "house" ? 0x8c5b48 : kind === "tree" ? 0x3f6b43 : kind === "lamp" ? 0x263338 : kind === "fence" ? 0x765335 : 0x273033, roughness: 0.85, metalness: kind === "barrel" || kind === "lamp" ? 0.65 : 0.05 }); let geo; let y = 0.6; if (kind === "stream") { const object=createStream(); object.position.set(point.x,0,point.z); reserveObject(object,kind,point); placedAssets.add(object); this.select(object); updatePreview(null,null); return; } if (kind === "rock") { geo = createChippedRockGeometry(Number(this.inputs.detail?.value || 1)); y = 0.55; } else if (kind === "crate") geo = new THREE.BoxGeometry(1.3, 1.1, 1.3); else if (kind === "building") { const object = createBuilding(); object.position.set(point.x, 0, point.z); object.userData.assetName = "new building"; object.userData.footprint=getSpec(kind).footprint; object.castShadow = object.receiveShadow = true; reserveObject(object,kind,point); placedAssets.add(object); this.select(object); updatePreview(null,null); return; } else if (kind === "road" || kind === "curvedRoad") { const object = createRoad(kind === "curvedRoad"); object.position.set(point.x,0,point.z); object.userData.footprint=getSpec(kind).footprint; object.userData.gridSize=GRID_SIZE; reserveObject(object,kind,point); placedAssets.add(object); this.select(object); updatePreview(null,null); return; } else if (kind === "car" || kind === "truck" || kind === "van") { const object = createVehicle(kind); object.position.set(point.x,0,point.z); object.userData.footprint=getSpec(kind).footprint; object.userData.gridSize=GRID_SIZE; reserveObject(object,kind,point); placedAssets.add(object); this.select(object); updatePreview(null,null); return; } else if (kind === "barrel") geo = new THREE.CylinderGeometry(0.6, 0.6, 1.2, 12); else if (kind === "house") { geo = new THREE.BoxGeometry(2.6, 1.8, 2.4); y = 0.9; } else if (kind === "tree") { geo = new THREE.ConeGeometry(1.0, 2.8, 7); y = 1.4; } else if (kind === "lamp") { geo = new THREE.CylinderGeometry(0.14, 0.2, 2.2, 8); y = 1.1; } else if (kind === "fence") { geo = new THREE.BoxGeometry(2.6, 1.0, 0.18); y = 0.5; } const object = new THREE.Mesh(geo, mat); object.position.set(point.x, y, point.z); object.userData.assetName = `new ${kind}`; object.userData.assetType = kind; object.userData.editable = true; object.userData.footprint = getSpec(kind).footprint; object.userData.gridSize = GRID_SIZE; if (kind === "lamp") { const bulb = new THREE.Mesh(new THREE.SphereGeometry(0.22, 12, 8), new THREE.MeshStandardMaterial({ color: 0xffd98a, emissive: 0xffa52f, emissiveIntensity: 4 })); bulb.position.y = 1.05; object.add(bulb); const lampLight = new THREE.PointLight(0xffb35c, 3, 8, 2); lampLight.position.y = 1.05; lampLight.castShadow = true; object.add(lampLight); object.userData.pointLight = lampLight; } object.userData.detail = Number(this.inputs.detail?.value || 1); object.castShadow = object.receiveShadow = true; reserveObject(object,kind,point); placedAssets.add(object); this.select(object); updatePreview(null,null); }, template(kind) { const blank = kind === "blank"; const beach = kind === "beach"; buildArea.visible = !beach; buildBorder.visible = !beach; tileSurface.visible = !beach; const yard = kind === "yard"; soil.visible = !blank && !beach; foundation.visible = yard; beachSand.visible = beach; beachWater.visible = beach; oceanWater.visible = beach; props.visible = vegetation.visible = border.visible = sign.visible = fireflies.visible = puddle.visible = model.group.visible = beacon.visible = yard; stream.visible = false;
    ...
    const updatePreview = (point, kind) => {
      ...
      const spec=getSpec(kind); const valid=placementValid(point,kind,editor?.selected?.rotation?.y||0,occupied);
