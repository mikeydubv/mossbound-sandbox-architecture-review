...
function createRoad(curved=false){const g=new THREE.Group();const m=new THREE.MeshStandardMaterial({color:0x30383a,roughness:.96});const road=new THREE.Mesh(new THREE.BoxGeometry(1,.08,3.8),m);road.position.y=.04;g.add(road);const stripe=new THREE.Mesh(new THREE.BoxGeometry(.08,.012,2.7),new THREE.MeshBasicMaterial({color:0xd3b861}));stripe.position.y=.085;g.add(stripe);if(curved){road.rotation.y= Math.PI/4;stripe.rotation.y=Math.PI/4}g.userData={assetName:curved?'curved road':'road',assetType:curved?'curvedRoad':'road',editable:true};return g}
