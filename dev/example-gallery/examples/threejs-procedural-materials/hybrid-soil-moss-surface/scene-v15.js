...
export default {
  ...
  async setup({ renderer, scene, camera, controls }) {
    ...
    const pointOnGrid = () => { const point = new THREE.Vector3(); if (!raycaster.ray.intersectPlane(ground, point)) return null; const x=snap(point.x), z=snap(point.z); return Math.abs(x)<=10 && Math.abs(z)<=10 ? point.set(x,0,z) : null; };
