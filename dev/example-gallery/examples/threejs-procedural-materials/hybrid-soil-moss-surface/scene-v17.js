...
export default {
  ...
  async setup({ renderer, scene, camera, controls }) {
    ...
    const sandMaterial = new THREE.ShaderMaterial({ uniforms: sandUniforms, side: THREE.DoubleSide, vertexShader: `varying vec2 vUv; void main(){vUv=uv; vec3 p=position; gl_Position=projectionMatrix*modelViewMatrix*vec4(p,1.0);}`, fragmentShader: `uniform float uTime; varying vec2 vUv; float hash(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7)))*43758.5453);} void main(){vec2 p=vUv*34.0; float grain=hash(floor(p*2.5)); float dune=sin(p.x*.55+sin(p.y*.31)*1.7)+sin(p.y*.42)*.5; vec3 sand=mix(vec3(.63,.38,.18),vec3(.88,.67,.38),grain*.22+.58); sand+=vec3(.055,.035,.012)*dune; gl_FragColor=vec4(sand,1.0);}` });
