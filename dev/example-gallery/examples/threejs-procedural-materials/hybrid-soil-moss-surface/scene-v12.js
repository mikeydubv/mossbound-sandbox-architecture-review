...
function createRoad(curved=false){const g=new THREE.Group();const m=new THREE.MeshStandardMaterial({color:0x30383a,roughness:.96});const line=new THREE.MeshBasicMaterial({color:0xd3b861}); if(!curved){const r=new THREE.Mesh(new THREE.BoxGeometry(1.8,.08,3.8),m);r.position.y=.04;g.add(r);const l=new THREE.Mesh(new THREE.BoxGeometry(.08,.012,2.7),line);l.position.y=.085;g.add(l);} else {const a=new THREE.Mesh(new THREE.BoxGeometry(1.8,.08,1.0),m);a.position.set(0,.04,-.9);const b=new THREE.Mesh(new THREE.BoxGeometry(1.0,.08,1.8),m);b.position.set(.4,.04,-.5);g.add(a,b);const la=new THREE.Mesh(new THREE.BoxGeometry(1.4,.012,.08),line);la.position.set(0,.085,-.9);const lb=new THREE.Mesh(new THREE.BoxGeometry(.08,.012,1.4),line);lb.position.set(.4,.085,-.5);g.add(la,lb);}g.userData={assetName:curved?'90 degree road corner':'road',assetType:curved?'curvedRoad':'road',editable:true};g.traverse(o=>{if(o.isMesh)o.receiveShadow=true});return g}
...
export default {
  ...
  async setup({ renderer, scene, camera, controls }) {
    ...
    const worldGrid = new THREE.GridHelper(40, 20, 0x4c8177, 0x294842);
    ...
    const updatePreview = (point, kind) => {
      ...
      const mesh=new THREE.Mesh(new THREE.BoxGeometry(w,0.08,d),new THREE.MeshBasicMaterial({color:valid?0x43e6a4:0xff4f69,transparent:true,opacity:.34,depthWrite:false})); mesh.position.set(point.x,.08,point.z); preview.add(mesh);
