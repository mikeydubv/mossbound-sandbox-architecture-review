...
function footprintCells(point, kind, rotation=0) {
  ...
  const quarter = Math.round(rotation / (Math.PI / 2)) % 2;
  const w = quarter ? spec.footprint[1] : spec.footprint[0];
  const d = quarter ? spec.footprint[0] : spec.footprint[1];
  const cells = [];
  const ox = point.x - ((w - 1) * GRID_SIZE) / 2;
  const oz = point.z - ((d - 1) * GRID_SIZE) / 2;
  for (let x=0; x<w; x++) for (let z=0; z<d; z++) cells.push(`${Math.floor((ox+x*GRID_SIZE)/GRID_SIZE)},${Math.floor((oz+z*GRID_SIZE)/GRID_SIZE)}`);
  return cells;
}
function placementValid(point, kind, rotation, occupied) { const spec=getSpec(kind); const q=Math.round(rotation/(Math.PI/2))%2; const w=q?spec.footprint[1]:spec.footprint[0], d=q?spec.footprint[0]:spec.footprint[1]; const cells=footprintCells(point,kind,rotation); const inBounds=cells.every((key)=>{const [x,z]=key.split(",").map(Number); return x>=-6&&x<=5&&z>=-6&&z<=5;}); return inBounds && cells.every((key) => !occupied.has(key)); }
...
export default {
  ...
  async setup({ renderer, scene, camera, controls }) {
    ...
    const reserveObject=(object,kind,point)=>{ object.userData.assetType=kind; object.userData.footprint=getSpec(kind).footprint; object.userData.gridSize=GRID_SIZE; object.userData.gridCells=footprintCells(point,kind,object.rotation.y); object.userData.editable=true; object.userData.anchor={x:point.x,z:point.z}; object.userData.assetName ||= `new ${kind}`; object.userData.gridPlaced=true; object.userData.gridCells.forEach((key)=>occupied.add(key)); };
